...
// Function to read the computer name from the registry
void ReadComputerName() {
    HKEY hKey;
    LSTATUS result;
    TCHAR computerName[MAX_VALUE_SIZE];
    ...
    if (result != ERROR_SUCCESS) {
        printf("Failed to open registry key for computer name. Error code: %ld\n", result);
        return;
    }
    ...
    if (result != ERROR_SUCCESS) {
        printf("Failed to read computer name. Error code: %ld\n", result);
    }
    else {
...
int main() {
    ...
    printf("Applications run on startup on local machine:");
    ReadRegistryValues(HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Run"));

    printf("Applications run on startup on current user:");
